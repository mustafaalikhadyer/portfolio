...
function CVPane({ title, icon: Icon, children, rotation, position, isMobile }: { title: string, icon: any, children: React.ReactNode, rotation: [number, number, number], position: [number, number, number], isMobile: boolean }) {
  return (
    <group position={position} rotation={rotation}>
      <mesh>
        ...
        <MeshTransmissionMaterial 
          ...
          color="#101010"
          anisotropicBlur={isMobile ? 0 : 0.1}
        />
        ...
        <Html transform distanceFactor={5} position={[0, 0, 0.05]} occlude={isMobile ? undefined : "blending"} className="pointer-events-none">
...
export default function NeuralCV() {
  ...
  return (
    <section className="relative h-[100vh] min-h-[900px] w-full py-20 bg-transparent flex flex-col items-center overflow-hidden">
      ...
      <div className={`w-full h-full cursor-grab active:cursor-grabbing relative z-20 transition-opacity duration-1000 ${mounted ? "opacity-100" : "opacity-0"}`}>
        ...
        {mounted && (
          <Canvas 
            camera={{ position: [0, 0, isMobile ? 25 : 18], fov: 45 }} 
            dpr={isMobile ? 1 : [1, 2]} 
            ...
            <Suspense fallback={null}>
              ...
              <Float speed={1} rotationIntensity={0.1} floatIntensity={0.2}>
                <group>
                  ...
                  <CVPane title="Profile" icon={BrainCircuit} position={[0, 0, 7]} rotation={[0, 0, 0]} isMobile={isMobile}>
                  ...
                  <CVPane title="Stack" icon={Zap} position={[7, 0, 0]} rotation={[0, Math.PI / 2, 0]} isMobile={isMobile}>
                  ...
                  <CVPane title="Jobb" icon={Briefcase} position={[0, 0, -7]} rotation={[0, Math.PI, 0]} isMobile={isMobile}>
                  ...
                  <CVPane title="Utbildningar" icon={GraduationCap} position={[-7, 0, 0]} rotation={[0, -Math.PI / 2, 0]} isMobile={isMobile}>
              ...
              <Stars radius={100} depth={50} count={isMobile ? 800 : 3000} factor={4} fade speed={0.5} />
            </Suspense>
          </Canvas>
        )}
      </div>
